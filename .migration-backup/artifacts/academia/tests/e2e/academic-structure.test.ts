...
import { createUserContext, makeAuthenticatedRequest, type TestUser } from './helpers';

// ─── Cleanup helpers (Sprint 2 models must be deleted before tenant/user) ────

async function cleanupAcademicStructure() {
  const ids = [TEST_DATA.schools.schoolA, TEST_DATA.schools.schoolB];
  ...
  describe('PATCH /api/v1/academic-years/[id] — update', () => {
    it('updates an academic year name', async () => {
      const yr = await prisma.academicYear.create({
        data: { schoolId: TEST_DATA.schools.schoolA, name: '2024-25', startDate: new Date('2024-04-01'), endDate: new Date('2025-03-31') },
      });
    ...
    it('cannot update a locked year', async () => {
      const yr = await prisma.academicYear.create({
        data: { schoolId: TEST_DATA.schools.schoolA, name: '2024-25', startDate: new Date('2024-04-01'), endDate: new Date('2025-03-31'), isLocked: true },
      });
  ...
  describe('POST /api/v1/academic-years/[id]/set-current', () => {
    it('performs atomic current-year swap', async () => {
      const [yr1, yr2] = await Promise.all([
        prisma.academicYear.create({ data: { schoolId: TEST_DATA.schools.schoolA, name: '2023-24', startDate: new Date('2023-04-01'), endDate: new Date('2024-03-31'), isCurrent: true } }),
        prisma.academicYear.create({ data: { schoolId: TEST_DATA.schools.schoolA, name: '2024-25', startDate: new Date('2024-04-01'), endDate: new Date('2025-03-31') } }),
      ]);
